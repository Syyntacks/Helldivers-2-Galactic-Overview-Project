...
class PlanetParser():
    ...
    def _fetch_and_combine(self):
        ...
        planet_stats = (details_data or {}).get("planet_stats") # In case empty dict
        ...
        # 2025-10-22: Placed into a try statement to handle errors
        try:
                # Dictionary Comprehension ****
                # Changed to handle if data is None
            planets_dict = {int(key): value for key, value in (planets_data or {}).items() if key.isdigit()}
            status_planets_dict = {int(key): value for key, value in (status_data or {}).items() if key.isdigit()}
